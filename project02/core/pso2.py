...
class Particle:
    ...
    def flip_multiple_items(self, flip_count):
        for _ in range(flip_count):
            if len(self.plan) > 0:
                item_idx = np.random.choice(len(self.plan))
                self.plan[item_idx] = 1 - self.plan[item_idx]

    def swap_adjacent_cities(self):
        if len(self.path) > 1:
